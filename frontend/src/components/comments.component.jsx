...
export const fetchComments = async ({
  ...
  try {
    ...
    const response = await axios.post(
      import.meta.env.VITE_SERVER_DOMAIN + "/comment/get-comments",
      { blog_id, skip }
    );
    if (response) {
      ...
      setParentCommentCountFunc((prev) => prev + response.data.length);
      if (comment_array === null) {
        res = { results: mappedComments };
      } else {
        res = { results: [...comment_array, ...mappedComments] };
      }
    }
    return res;
  } catch (err) {
  ...
  return (
    ...
    >
      <div className="relative">
        <h1 className="tetx-xl font-medium">Comments</h1>
